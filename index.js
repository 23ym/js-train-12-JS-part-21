...
function createDictionary(entries) {
  const dictionary = new Map(); // Створення порожнього словника
  entries.forEach(([key, value]) => {// Використання методу forEach для перебору масиву пар ключ-значення
    dictionary.set(key, value); // Додавання пари ключ-значення до словника за допомогою методу set
  });
  return dictionary;
}
...
function getKeysStartingWith(dictionary, letter) {
  const keysStartingWithLetter = []; // Оголошення порожнього масиву для збереження ключів
  for (const key of dictionary.keys()) { // Перебір ключів словника за допомогою циклу for...of
  ...
  return keysStartingWithLetter; // Повертаємо масив
}
...
function addKeyValuePairs(dictionary, entries) {
  let added = 0;
  let rejected = 0; // Ведемо статистику доданих та відхилених ключів, для цього створемо змінні added та rejected з початковими значеннями 0
  entries.forEach(([key, value]) => { // Використовуємо метод forEach для перебору масиву пар ключ-значення
    if (!dictionary.has(key)) { // Перевіряємо, чи словник вже містить такий ключ за допомогою методу has
      dictionary.set(key, value); 
      added++; // Якщо ключ є унікальним, додаємо його до словника за допомогою методу set та збільшимо added на 1
    } else { 
      rejected++; 
...
function transformDictionary(dictionary) {
  const keys = []; 
  const values = []; 
  const entries = []; // Створюємо порожні масиви для ключів (змінна keys), значень (змінна values) і пар ключ-значення (змінна entries)
  for (const key of dictionary.keys()) { // Використовуємо цикл for...of і метод keys для перебору ключів словника
    keys.push(key); 
  } // Додаємо кожен ключ до масиву keys за допомогою методу push
  for (const value of dictionary.values()) { // Використовуємо цикл for...of і метод values для перебору значень словника
    values.push(value); // Додаємо кожне значення до масиву values за допомогою методу push
  }
    for (const entry of dictionary.entries()) {  // Використовуємо цикл for...of і метод entries для перебору пар ключ-значення словника
      entries.push(entry); // Додаємо кожну пару ключ-значення до масиву entries за допомогою методу push
    }
...
function checkPresence(dictionary, keys) {
  const precence = []; // Створюємо порожній масив для збереження результатів перевірки
  keys.forEach((key) => { // Використовуємо цикл forEach для перебору масиву ключів
    precence.push(dictionary.has(key)); 
   }); // Додаємо результат перевірки (true або false) до масиву presence при наявності ключа у словнику
  return precence; // Повертаємо масив
}
...
function getFilteredDictionarySize(dictionary, filter) {
  const filteredDictionary = new Map(); // Створюємо новий Map об'єкт для зберігання елементів, що відповідають фільтру
...
function sortByValues(dictionary) {
  const entries = [...dictionary.entries()]; // Конвертуємо словник в масив пар ключ-значення за допомогою оператора деструктурізації
  entries.sort((a, b) => b[1] - a[1]); // Сортуємо масив пар ключ-значення за значеннями в порядку спадання
  const sortedDictionary = new Map(entries); // Конвертуємо відсортований масив пар ключ-значення назад у словник
...
function resetDictionary(dictionary, maxSize) {
  ...
  // Якщо кількість елементів більша ніж максимально допустима, очищуємо словник за допомогою методу clear
    return true; // Повертаємо true, оскільки словник був очищений
  } else { 
    return false; 
...
function convertDictionaryToSet(dictionary) {
  ...
  dictionary.forEach((value,key) =>  { 
    if (typeof key === 'string') { // Якщо ключ є рядком, переводимо його в верхній регістр і додаємо до множини
      resultSet.add(key.toUpperCase()); 
    } else if (typeof key === 'number') {
      resultSet.add(key + 1);  // Якщо ключ є числом, збільшуємо його на 1 і додаємо до множини
...
function convertSetToDictionary(set) {
  ...
  set.forEach((element) => { 
    ...
    dictionary.set(element, firstChar.charCodeAt(0)); 
